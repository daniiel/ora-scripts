...
set feedback off
--------------------------------------------------------------------------------------------------------------
-- Script: ConWin | Configures the SQL Plus console in Windows.
...
-- Params: instance_type PROD|TEST
--------------------------------------------------------------------------------------------------------------
prompt ------------------------------------------------------------------------------|
prompt ConWin | Configures the SQL Plus console in Windows.
prompt ------------------------------------------------------------------------------|
prompt Use:     conwin instance_type (prod|test)
prompt Example: conwin prod
prompt ------------------------------------------------------------------------------|

-- Special columns definition.
def _def_instance = ""
def _def_color    = ""
def _def_sid      = ""
def _def_serial   = ""
...
-- Special columns for info like instance and color of the window.
col c_instance head NAME for a8 new_value _def_instance
col c_color    noprint         new_value _def_color
col c_sid      noprint         new_value _def_sid
col c_serial   noprint         new_value _def_serial
col c_spid     noprint         new_value _def_spid
...
select ltrim (ses.sid)     c_sid,
      ltrim (ses.serial#) c_serial,
      ltrim (pro.spid)    c_spid
from   gv$session ses,
      gv$process pro
where  ses.paddr = pro.addr
  and  ses.sid   = (select sid from gv$mystat where rownum = 1);
 
-- MSDOS window title.
host &_def_color
host title &_user@&_connect_identifier [ins_&_def_instance  sid_&_def_sid  ser#_&_def_serial  spid_&_def_spid]

-- Special columns release.
undef _def_instance _def_color _def_sid _def_serial _def_spid 1
set feedback on
